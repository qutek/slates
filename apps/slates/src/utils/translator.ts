...
class Translator implements ITranslator {
   ...
   */
  async updateTKK() {
    const response = await fetch(this.HOME_PAGE).then((res) => res.json());
  ...
  parseBetterResult(response: GoogleTranslateResponse) {
    ...
    if (response.sentences) {
      ...
      for (
        ;
        i < response.sentences.length && response.sentences[i].trans;
        i++
      ) {
  ...
  detect(text: string) {
    let detectOnce: any = async () => {
       ...
       */
      const response = await fetch(this.generateDetectURL(text));

      if (response.status === 200) {
        const data = await response.json();
        return this.parseDetectResult(data);
      }
  ...
  translate(text: string, from: string, to: string) {
    let translateOnce: any = async () => {
       ...
       * not throw error when status code is less than 500.
       */
      const response = await fetch(this.generateTranslateURL(text, from, to));

      if (response.status === 200) {
        const data = await response.json();
        let result = this.parseTranslateResult(data);
        return result;
      }
  ...
  getAudioUrl(text: string, language: string, speedValue: string) {
    return `${this.fallBacking ? this.FALLBACK_TTS_URL : this.TTS_URL
        ...
        language,
      )}&ttsspeed=${speedValue}&tk=${this.generateTK(
        text,
        this.TKK[0],
        this.TKK[1],
      )}`;
